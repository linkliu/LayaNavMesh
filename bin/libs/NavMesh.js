...
(function (NavMesh) {
    class Astar {
        ...
        static heap() {
            return new NavMesh.BinaryHeap(function (node) {
                return node.f;
            });
        }
        ...
        static heuristic(pos1, pos2) {
            return NavMesh.Vector3.distanceToSquared(pos1, pos2);
        }
...
(function (NavMesh) {
    class Channel {
        ...
        vequal(a, b) {
            return NavMesh.Vector3.distanceToSquared(a, b) < 0.00001;
        }
        stringPull() {
...
(function (NavMesh) {
    ...
    NavMesh.Geometry = Geometry;
})(NavMesh || (NavMesh = {}));
/*
    NavMesh.js - Navigation mesh toolkit for Laya base on PatrolJS
...
(function (NavMesh) {
    ...
    function buildNodesByJson(json) {
        ...
        for (let i = 0; i < ii.length / 5; i++) {
            faces.push(new NavMesh.Face(ii[i * 5 + 1], ii[i * 5 + 2], ii[i * 5 + 3]));
        }
        ;
        let p = [];
        for (let i = 0; i < p2.length; i += 3) {
            p.push(new NavMesh.Vector3(p2[i], p2[i + 1], p2[i + 2]));
        }
        ;
        let g = new NavMesh.Geometry();
        g.faces = faces;
        g.vertices = p;
        let zoneNodes = buildNodes(g);
        return zoneNodes;
    }
    NavMesh.buildNodesByJson = buildNodesByJson;
    function buildNodes(geometry) {
        let navigationMesh = NavMesh.Patroll.buildNavigationMesh(geometry);
        let zoneNodes = NavMesh.Patroll.groupNavMesh(navigationMesh);
        return zoneNodes;
    }
    NavMesh.buildNodes = buildNodes;
    function setZoneData(zone, data) {
        NavMesh.zoneNodes[zone] = data;
    }
    NavMesh.setZoneData = setZoneData;
    function getGroup(zone, position) {
        ...
        for (let i = 0, len = NavMesh.zoneNodes[zone].groups.length; i < len; i++) {
            ...
            for (let j = 0, len2 = group.length; j < len2; j++) {
                const node = group[j];
                let measuredDistance = NavMesh.Vector3.distanceToSquared(node.centroid, position);
    ...
    NavMesh.getGroup = getGroup;
    function getRandomNode(zone, group, nearPosition, nearRange) {
        if (!NavMesh.zoneNodes[zone])
            return new NavMesh.Vector3();
        nearPosition = nearPosition || null;
        nearRange = nearRange || 0;
        let candidates = [];
        let polygons = NavMesh.zoneNodes[zone].groups[group];
        for (let i = 0, len = polygons.length; i < len; i++) {
            const p = polygons[i];
            if (nearPosition && nearRange) {
                if (NavMesh.Vector3.distanceToSquared(nearPosition, p.centroid) < nearRange * nearRange) {
        ...
        if (candidates.length > 0) {
            let index = NavMesh.random(candidates.length);
            candidates[index];
            return candidates[index];
        }
    }
    NavMesh.getRandomNode = getRandomNode;
    function findPath(startPosition, targetPosition, zone, group) {
        ...
        for (let i = 0, len = allNodes.length; i < len; i++) {
            ...
            let measuredDistance = NavMesh.Vector3.distanceToSquared(node.centroid, startPosition);
            if (measuredDistance < distance) {
                closestNode = node;
                distance = measuredDistance;
            }
        ...
        for (let i = 0, len = allNodes.length; i < len; i++) {
            const node = allNodes[i];
            let measuredDistance = NavMesh.Vector3.distanceToSquared(node.centroid, targetPosition);
            if (measuredDistance < distance &&
                NavMesh.Vector3.isVectorInPolygon(targetPosition, node, vertices)) {
                farthestNode = node;
                distance = measuredDistance;
            }
        }
        ...
        let paths = NavMesh.Astar.search(allNodes, closestNode, farthestNode);
        ...
        // We got the corridor
        // Now pull the rope
        let channel = new NavMesh.Channel();
        ...
        for (let i = 0, len = channel.path.length; i < len; i++) {
            ...
            let vec = new NavMesh.Vector3(c.x, c.y, c.z);
    ...
    class Patroll {
        static computeCentroids(geometry) {
            ...
            for (f = 0, fl = geometry.faces.length; f < fl; f++) {
                ...
                face.centroid = new NavMesh.Vector3(0, 0, 0);
                NavMesh.Vector3.add(face.centroid, geometry.vertices[face.a], face.centroid);
                NavMesh.Vector3.add(face.centroid, geometry.vertices[face.b], face.centroid);
                NavMesh.Vector3.add(face.centroid, geometry.vertices[face.c], face.centroid);
                NavMesh.Vector3.scale(face.centroid, 1 / 3, face.centroid);
            }
        }
        ;
        ...
        static buildPolygonNeighbours(polygon, navigationMesh) {
            ...
            for (let i = 0, len = navigationMesh.polygons.length; i < len; i++) {
                ...
                if (NavMesh.Vector3.distanceToSquared(polygon.centroid, navigationMesh.polygons[i].centroid) > 100 * 100)
                    continue;
